    ...
    var colpick = function() {
        var
            ...
            fillHSXFields = function(hsx, cal) {
                $(cal).data('colpick').fields.eq(4).val(Math.round(hsx.h)).end().eq(5).val(Math.round(hsx.s)).end().eq(6).val(Math.round(hsx.x)).end();
            },
            ...
            show = function(ev) {
                ...
                var pos = $(this).offset();
                var top = pos.top - this.offsetHeight - cal.data('colpick').height * 1.52;
                var left = pos.left - cal.data('colpick').height;
            ...
            setColor: function(col, setCurrent) {
                ...
                return this.each(function() {
                    if ($(this).data('colpickId')) {
                        ...
                        setSelector(col, cal.get(0));
                        setNewColor(col, cal.get(0));
                        cal.data('colpick').onChange.apply(cal.parent(), [col, cal.data('colpick').hsl ? hslToHex(col) : hsbToHex(col), cal.data('colpick').hsl ? hslToRgb(col) : hsbToRgb(col), cal.data('colpick').el, 1]);
                        if (setCurrent) {
                            setCurrentColor(col, cal.get(0));
                        }
                    }
